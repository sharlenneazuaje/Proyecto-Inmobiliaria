...
function mostrarModificarModal(id) {
    fetch(`/proyecto_java/GestionUsuariosServlet?id=${id}`)
        .then(response => response.json())
        .then(usuario => {
            //console.log(usuario)
...
function guardarModificacion() {
    ...
    fetch('/proyecto_java/GestionUsuariosServlet', {
        method: 'PUT',
        headers: {
...
function eliminarUsuario(id) {
    ...
    if (confirm('¿Está seguro de que desea eliminar este usuario?')) {
        fetch(`/proyecto_java/GestionUsuariosServlet?id=${id}`, { method: 'DELETE' })
            .then(response => response.json())
            .then(data => {
                console.log(data.exito)
                if (data.exito) {
